...
- **Deterministic test order**: Vitest `sequence.concurrent: false` and `sequence.shuffle: false` so tests run in a fixed order and without interleaving, reducing flakiness and making failures easier to reproduce.
